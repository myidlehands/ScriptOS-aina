...
export default function App() {
  ...
  useEffect(() => {
    ...
    // 2. Initialize Google Auth Logic
    Auth.initGoogleAuth(async (response: any) => {
        if (response && response.access_token) {
            const token = response.access_token;
            console.log("Authenticated! Token acquired.");
            
            // Fetch User Data
            const myChannel = await YouTube.fetchMyChannel(token);
            const myAnalytics = await YouTube.fetchMyAnalytics(token);
            
            if (myChannel) {
                const currentProfile = Storage.getUserProfile() || { identity: { brandVoice: '', targetAudience: '', manifesto: '' } };
                const updatedProfile: UserProfile = {
                    ...currentProfile,
                    ...myChannel,
                    accessToken: token,
                    analytics: myAnalytics || undefined
                };
  ...
  const renderView = () => {
    switch (view) {
      case 'DASHBOARD':
        return <Dashboard scripts={scripts} onNavigate={setView} lang={lang} userProfile={userProfile} />;
      case 'TREND_HUNTER':
      ...
      case 'PROFILE':
        return <ChannelProfile lang={lang} userProfile={userProfile} onProfileUpdate={refreshProfile} />;
      case 'STUDIO':
        return activeScriptId 
          ? <Studio scriptId={activeScriptId} onClose={() => setView('DASHBOARD')} onUpdate={refreshScripts} lang={lang} />
          : <Dashboard scripts={scripts} onNavigate={setView} lang={lang} userProfile={userProfile} />;
      default:
        return <Dashboard scripts={scripts} onNavigate={setView} lang={lang} userProfile={userProfile} />;
    }
  };
